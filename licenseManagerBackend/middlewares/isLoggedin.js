...
module.exports.isLoggedIn = async (req,res,next) => {
    ...
    try {
        ...
        req.user = {email:user.user.email,username:user.user.username,user_id:user.user.user_id,isAdmin:user.user.isAdmin,isSuperAdmin:user.user.isSuperAdmin};
        next();
    } catch (error) {
...
module.exports.isSuperAdmin = async (req,res,next)=>{
    ...
    try {
        ...
        if(!user.isSuperAdmin) return res.status(401).send({error:"Only Super Admins are allowed"});
